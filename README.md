...
知识库后端使用 PostgreSQL 15 + pgvector。普通过滤字段使用 B-tree，1024 维
Embedding 使用 HNSW + cosine 索引。文件上传后会按“抽取式摘要 → 语义边界 →
递归字符串兜底”生成 chunks，再通过本地 Ollama 的 `bge-m3` 生成向量。检索时
HNSW 先召回 Candidate Top-20，随后由 TEI 托管的
`BAAI/bge-reranker-v2-m3` Cross-Encoder 精排为 Final Top-6。API 响应保留
`vector_score`、`rerank_score` 和 `retrieval_mode`，便于确认精排是否生效。

```bash
# PostgreSQL/pgvector + Redis
docker compose up -d postgres redis

# 可选：checkpoint 准备完成后离线启动 Embedding + Reranker
...
curl -X POST http://localhost:8081/rerank \
  ...
  -d '{"query":"如何重建知识库索引？","texts":["调用 reindex 接口","删除数据库"],"raw_scores":false}'
```

模型 checkpoint 已实际下载并校验，使用项目可见目录，不存放在 Docker named
volume。目前共有三份本地模型目录：

- `./model-cache/ollama/models/`：Ollama 原生 `bge-m3` cache，包含 manifest、
  config、license 和 1,157,671,200 字节模型 blob；运行 Ollama/后端 Embedding 时
  实际使用这一份。
- `./model-cache/huggingface/bge-m3/`：Hugging Face 原始 `bge-m3` checkpoint，
  主权重 `pytorch_model.bin` 为 2,271,145,830 字节；仅供 Transformers 直接加载、
  格式转换或离线备份，当前 Compose/Ollama 运行链路不会读取这一份。
- `./model-cache/huggingface/bge-reranker-v2-m3/`：TEI 直接加载的 Reranker
  checkpoint，`model.safetensors` 为 2,271,071,852 字节；Compose 将其映射为
  `/data/bge-reranker-v2-m3`。

对应绝对路径都是 `<项目根目录>/model-cache/...`。当前工作区可用以下命令查看实际
绝对位置和已记录的完整 SHA-256：

在 PowerShell 中可用以下命令查看当前机器的实际绝对路径：

```powershell
Resolve-Path .\model-cache\ollama\models
Resolve-Path .\model-cache\huggingface\bge-m3
Resolve-Path .\model-cache\huggingface\bge-reranker-v2-m3
Get-Content .\model-cache\CHECKSUMS.sha256
```

Linux/macOS 可运行 `cat ./model-cache/CHECKSUMS.sha256` 查看清单，并在项目根目录执行
`sha256sum -c ./model-cache/CHECKSUMS.sha256` 复核权重、Ollama config/license 和
manifest。三个 checkpoint 当前均已完整落盘，因此 Docker/TEI 镜像可用时可以直接
离线挂载启动，不需要再次从模型仓库下载权重。

没有 Docker 时也可以预先下载或更新 checkpoint：

```powershell
# 使用本机 Ollama，把原生模型清单和 blobs 写入项目缓存
$env:OLLAMA_MODELS = "$PWD\model-cache\ollama\models"
ollama pull bge-m3

# 使用 Hugging Face CLI 下载完整快照；再次执行即可检查并更新文件
hf download BAAI/bge-reranker-v2-m3 `
  --local-dir .\model-cache\huggingface\bge-reranker-v2-m3
```

Linux/macOS 对应写法：

```bash
OLLAMA_MODELS="$PWD/model-cache/ollama/models" ollama pull bge-m3
hf download BAAI/bge-reranker-v2-m3 \
  --local-dir ./model-cache/huggingface/bge-reranker-v2-m3
```

Compose 将 `model-cache/ollama` 挂载为 `/root/.ollama`，将
`model-cache/huggingface` 只读挂载为 `/data`；因此 checkpoint 完整后即使断网也可
启动。Reranker 缓存不完整时 TEI 会启动失败，需要先重新执行 `hf download`。
`model-cache/` 已加入 `.gitignore`，模型权重体积很大且可能受许可证约束，禁止提交
到 Git；更新模型也只应更新本地缓存。更新 `bge-m3` Embedding 后需要对已有知识库
执行 reindex；只更新 Reranker checkpoint 不需要重建向量索引。

`bge-reranker-v2-m3` 是约 0.6B 参数的 CPU Cross-Encoder，建议为 Reranker
预留数 GB 内存。若机器资源不足，可设置 `rag.reranker_enabled: false` 或
`AGENTHUB_RAG_RERANKER_ENABLED=false`，此时结果保持 HNSW 向量顺序。
Reranker 超时、返回非 2xx 或响应索引非法时也会自动降级，不会导致 RAG 请求失败。

已有知识库可通过 `POST /api/knowledge-bases/:id/reindex` 重建索引；语义检索接口为
`GET /api/knowledge-bases/:id/rag/search?q=<query>&limit=6`。配置项和可用环境变量见
`src/backend/config/config.example.yaml`；常用覆盖项包括
`AGENTHUB_RAG_ENABLED`、`AGENTHUB_RAG_OLLAMA_URL`、`AGENTHUB_RAG_MODEL`、
`AGENTHUB_RAG_TOP_K`、`AGENTHUB_RAG_CANDIDATE_TOP_N`、
`AGENTHUB_RAG_BATCH_SIZE`、`AGENTHUB_RAG_RERANKER_ENABLED`、
`AGENTHUB_RAG_RERANKER_URL`、`AGENTHUB_RAG_RERANKER_MODEL` 和
`AGENTHUB_RAG_RERANKER_TIMEOUT_SECONDS`。
从旧模型切换到 BGE-M3 后必须调用重建接口，使已有 chunks 全部替换为 1024 维
`bge-m3` 向量；不同模型的向量不会混合召回。
