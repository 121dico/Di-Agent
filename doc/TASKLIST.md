...
- [x] [图表查询与结果对应记录](task/report-chart-provenance.md)：V1.2 聚合请求、SQL、映射版本、执行快照和管理员核验重跑；已上线并通过真实调用验证。
...
| # | 任务 | 详情 | 依赖 | 状态 |
|---|------|------|------|------|
| R-C2 | V1.2 三图双模式及离群标记（已上线，修正版视觉验收待完成） | [双模式验收](task/report-chart-modes.md) | R-C1 | [~] |
| R-T1 | 可复现模板一与 V1.2 报表 | [报表模板验收](task/report-template-v12.md) | 现有报表 | [~] |
| R-C1 | V1.2 全量、订单人群及每日新增分区 | [人群报表验收](task/report-v12-cohorts.md) | R-T1，日粒度数据 | [~] |
...
| B94 | ChatView 仅首次挂载读取 URL 参数，浏览器前进/后退无法切换对话 | P2 | [x] |
...
| UI-07 | 多处硬编码颜色不跟随主题 | P2 | [x] |
| UI-08 | AuthLayout 固定宽度窄屏溢出 | P2 | [x] |
| UI-09 | 多处内联 style 无法被暗色主题覆盖 | P2 | [x] |
...
| MISS-003 | 设置页面实现 | 大部分前端本地(主题按钮无功能) | [x] |

### P1 — 重要功能缺失

| ID | 功能 | 后端状态 | 状态 |
|----|------|----------|------|
| MISS-004 | 群成员角色管理 UI | 部分已有 | [x] |
...
| UX-13 | 消息列表不支持键盘快捷键（Esc关闭面板等） | P2 | [x] |
...
| IM-015 | 多标签页 WS 同步（BroadcastChannel 跨 tab 状态一致） | 纯前端 | — | [ ] |
...
## 2026-09-08 本地 Skills 与执行链路

- [x] Skills 页面改为顶部 Agent 选择、本地技能优先；平台库保留为次级操作。
- [x] 本地技能仅同步功能、用法和位置索引；本机按需加载正文。
- [x] 回复旁独立执行链路面板，展示真实 Skill/MCP 成功与失败并支持历史回放。
- [x] 隔离环境真实 Codex 验收；前端 238 项、npm daemon 214 项测试通过。后端保留已验证的既有部署路径测试失败。线上发布未执行。
- [x] Harness 风格彩色轨迹：输入/模型输出/Skill/MCP 泳道、搜索筛选、色条定位、真实观测时间与旧历史顺序回放；真实调用及刷新复验通过。

## 2026-09-14 V1.2 数据验证

- [x] [计划提价前后固定人群核验](task/station-price-response-verification.md)：真实API描述性分析完成，辅助板块已后置并部署，桌面真实查询验收通过；缺少实付/电量/对照，不能作因果判断。
...
## 2026-09-15 复购默认加载与判分核验

- [x] [复购自动展示与高价敏判分依据](task/station-score-evidence.md)：已部署开发机，真实快照及管理员浏览器验收通过；同次修复订单笔数加载空态、成功档位复用与TTL边界，首次冷查询仍取决于上游耗时。
