...
- Bash 工具的工作目录默认为项目根 `/Users/shallow/Desktop/repo/di-agent`
...
2. 每次执行 Go 命令时，command 必须以 `cd /Users/shallow/Desktop/repo/di-agent/src/backend &&` 开头
3. 每次执行前端 npm/node 命令时，command 必须以 `cd /Users/shallow/Desktop/repo/di-agent/src/frontend &&` 开头
4. 写 Bash tool call 时，先写完 command 参数，确认包含 cd，再写 description
5. 连续失败 2 次同一条命令时，立即检查 command 内容而非重试

## 相关项目结构
```
/Users/shallow/Desktop/repo/di-agent/     ← 项目根（默认工作目录）
