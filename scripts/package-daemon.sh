...
# 产物：agenthub-daemon-bundle.tar.gz（daemon + 依赖 ws，解压即用，无需 npm/外网）
# 依赖：本机可访问 npm 源（默认走 npmmirror，可用 NPM_REGISTRY 覆盖）
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT_DIR="$ROOT/src/backend/downloads"
BUNDLE_NAME="agenthub-daemon-bundle.tar.gz"
...
test -f "$TMP_DIR/node_modules/@hust-agenthub/daemon/bin/agenthub-daemon.js" || {
