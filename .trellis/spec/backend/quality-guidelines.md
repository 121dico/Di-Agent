...
### Streaming Watchdog Timing

**Scope / Trigger**: Applies when changing Agent dispatch timeouts or streaming-message cleanup.

**Contracts**:
- The streaming watchdog stale threshold must be longer than the maximum Dispatcher task timeout.
- A CLI that emits no incremental chunks, such as Codex `exec`, must remain `streaming` while its task is legitimately running.
- Dispatcher timeout/finalization owns normal task failure; the watchdog only cleans orphaned placeholders.

**Tests Required**:
- Assert the default watchdog age exceeds the 30-minute Dispatcher timeout.
- Assert explicit short ages still mark truly stale messages in focused watchdog tests.

**Wrong vs Correct**:
```go
// Wrong: marks a 125-second Codex response as failed after one minute.
NewStreamingWatchdog(repo, logger, 60*time.Second, 10*time.Second)

// Correct: runs after the shared Agent task timeout.
NewStreamingWatchdog(repo, logger, service.DefaultStreamingWatchdogMaxAge, 10*time.Second)
```

### Agent Task Timeout Contract
...
#### 3. Contracts
- The default daemon and backend Agent task timeout is 30 minutes.
- Missing, non-numeric, zero, negative, or unsafe daemon overrides fall back to 30 minutes.
- Every outer `context.WithTimeout` around Agent execution must use `DefaultAgentTaskTimeout`; a shorter wrapper silently defeats the shared limit.
- The streaming watchdog threshold must remain strictly longer than the Agent task timeout.
- Persistent CLI adapters must clear the active turn timer on success and process close, not only when the timer fires.

#### 4. Validation & Error Matrix
- Valid positive integer override -> use that millisecond value.
- Invalid override -> use `1800000` without preventing daemon startup.
- Agent exceeds the effective timeout -> return a timeout failure and finalize the streaming message.
- Agent completes or exits before timeout -> clear its timer immediately.

#### 5. Good / Base / Bad Cases
- Good: a 12-minute Claude task completes under the shared 30-minute limit and its timer is released on `TURN_END`.
- Base: no environment override is provided; daemon and backend both use 30 minutes.
- Bad: an async message wrapper still uses 180 seconds, so it cancels a healthy task before the daemon limit.

#### 6. Tests Required
- Daemon tests assert default/override parsing and timer cleanup after a completed persistent turn.
- Backend tests assert shared timeout usage and `DefaultStreamingWatchdogMaxAge > DefaultAgentTaskTimeout`.
- Run focused streaming/watchdog tests after changing either constant.

#### 7. Wrong vs Correct
```go
// Wrong: a hidden outer limit defeats the shared task timeout.
ctx, cancel := context.WithTimeout(ctx, 180*time.Second)

// Correct: all Agent execution wrappers share one contract.
ctx, cancel := context.WithTimeout(ctx, DefaultAgentTaskTimeout)
```
