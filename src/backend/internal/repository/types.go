...
type KnowledgeStore interface {
	...
	ReplaceChunks(ctx context.Context, kbID, fileID string, chunks []model.KnowledgeChunk) error
	UpdateFileRAGStatus(ctx context.Context, kbID, fileID, status, ragError string, chunkCount int, indexedAt *time.Time) error
	SearchChunks(ctx context.Context, kbID, userID, embeddingModel string, embedding []float32, limit int) ([]model.KnowledgeChunkSearchResult, error)
