...
import (
	...
	middleware "github.com/121dico/Di-Agent/src/backend/internal/middleware"
...
// SearchRAG performs semantic chunk retrieval, with keyword fallback when the
// local embedding service is unavailable.
func (h *KnowledgeHandler) SearchRAG(c *gin.Context) {
	userID := middleware.GetUserID(c)
	kbID := c.Param("id")
	query := strings.TrimSpace(c.Query("q"))
	if kbID == "" || query == "" {
		middleware.ErrorResponse(c, http.StatusBadRequest, 40078, "缺少知识库 ID 或查询内容")
		return
	}
	limit := 0
	if rawLimit := strings.TrimSpace(c.Query("limit")); rawLimit != "" {
	...
	results, err := h.svc.SearchRAG(c.Request.Context(), userID, kbID, query, limit)
	if err != nil {
		if err == service.ErrKBNotFound || err == service.ErrKBNoPermission {
			status, code := http.StatusNotFound, 40474
			if err == service.ErrKBNoPermission {
				status, code = http.StatusForbidden, 40369
			}
			middleware.ErrorResponse(c, status, code, err.Error())
			return
		}
		middleware.ErrorResponse(c, http.StatusInternalServerError, 50073, "RAG 检索失败")
		return
	}
	middleware.SuccessResponse(c, results)
}

// Reindex rebuilds all extractable files in a knowledge base using the current
// chunking and embedding configuration.
func (h *KnowledgeHandler) Reindex(c *gin.Context) {
	userID := middleware.GetUserID(c)
	kbID := c.Param("id")
	if kbID == "" {
		middleware.ErrorResponse(c, http.StatusBadRequest, 40079, "缺少知识库 ID")
		return
	}
	result, err := h.svc.Reindex(c.Request.Context(), userID, kbID)
	if err != nil {
		if err == service.ErrKBNotFound || err == service.ErrKBNoPermission {
			status, code := http.StatusNotFound, 40475
			if err == service.ErrKBNoPermission {
				status, code = http.StatusForbidden, 40370
			}
			middleware.ErrorResponse(c, status, code, err.Error())
			return
		}
		middleware.ErrorResponse(c, http.StatusInternalServerError, 50074, "重建 RAG 索引失败")
		return
	}
