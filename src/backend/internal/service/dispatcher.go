...
// ContextUsageRecorder keeps Dispatcher independent from the concrete context meter service.
type ContextUsageRecorder interface {
	RecordDispatch(ctx context.Context, in RecordContextUsageInput) (*model.AgentSession, error)
...
type DispatcherDeps struct {
	...
	ContextMeter  ContextUsageRecorder
}
...
// SetContextMeter 注入跨派发路径共享的上下文用量台账。
func (d *Dispatcher) SetContextMeter(meter ContextUsageRecorder) {
	d.deps.ContextMeter = meter
}
...
func (d *Dispatcher) dispatchPlanCore(
	...
	if in.ForceFreshSession {
		dispatchData["force_fresh_session"] = true
	}
	if in.SessionGeneration > 0 {
		dispatchData["session_generation"] = in.SessionGeneration
	}
	if in.CheckpointID != "" {
		dispatchData["checkpoint_id"] = in.CheckpointID
	}
	if handle != nil {
	...
	if err != nil {
		d.recordContextUsage(ctx, in, prompt, contextMessages, "")
	...
	d.recordContextUsage(ctx, in, prompt, contextMessages, task.Result)
	d.recordCLISessionID(ctx, in, task.CLISessionID)
...
func (d *Dispatcher) recordContextUsage(ctx context.Context, in DispatchInput, prompt, contextMessages, output string) {
	if d.deps.ContextMeter == nil || in.Agent == nil {
		return
	}
	_, err := d.deps.ContextMeter.RecordDispatch(context.WithoutCancel(ctx), RecordContextUsageInput{
		ConversationID: in.ConvID,
		AgentID:        in.Agent.ID,
		CLITool:        in.Agent.CLITool,
		Prompt:         prompt,
	...
	if err != nil {
		slog.Warn("dispatcher: record context usage failed", "conversation_id", in.ConvID, "agent_id", in.Agent.ID, "error", err)
	}
}

func (d *Dispatcher) recordCLISessionID(ctx context.Context, in DispatchInput, cliSessionID string) {
	if d.deps.ContextMeter == nil || in.Agent == nil || cliSessionID == "" {
		return
	}
	if err := d.deps.ContextMeter.SetCLISessionID(context.WithoutCancel(ctx), in.ConvID, in.Agent.ID, cliSessionID); err != nil {
		slog.Warn("dispatcher: record CLI session id failed", "conversation_id", in.ConvID, "agent_id", in.Agent.ID, "error", err)
	}
}
...
//   - 达到 DefaultAgentTaskTimeout → ErrMsgAgentTimeout
