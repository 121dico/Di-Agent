...
// SetContextMeter injects the durable per-session usage ledger.
func (s *MessageService) SetContextMeter(meter ContextUsageRecorder) {
	s.contextMeter = meter
}
...
func (s *MessageService) createAgentReply(ctx context.Context, convID, userID, agentID, userContent, contextMessages string, replyTo *string, runtimeConfig ...model.AgentRuntimeConfig) (*model.Message, error) {
	...
	if ch == nil {
		s.recordContextUsage(ctx, convID, agent, userContent, contextMessages, "")
		if _, ferr := FinalizeStreamingPipeline(ctx, pipelineDeps, handle, FinalizeStreamingPipelineOptions{
			Status: model.MessageStatusError,
		}); ferr != nil {
	...
	s.recordContextUsage(ctx, convID, agent, userContent, contextMessages, result.Result)
	if s.contextMeter != nil && result.CLISessionID != "" {
		if err := s.contextMeter.SetCLISessionID(context.WithoutCancel(ctx), convID, agent.ID, result.CLISessionID); err != nil {
			slog.Warn("message service: record CLI session id failed", "conversation_id", convID, "agent_id", agent.ID, "error", err)
		}
	}
...
func (s *MessageService) recordContextUsage(ctx context.Context, convID string, agent *model.Agent, prompt, contextMessages, output string) {
	if s.contextMeter == nil || agent == nil {
		return
	}
	_, err := s.contextMeter.RecordDispatch(context.WithoutCancel(ctx), RecordContextUsageInput{
	...
	if err != nil {
		slog.Warn("message service: record context usage failed", "conversation_id", convID, "agent_id", agent.ID, "error", err)
	}
}
...
func (s *MessageService) asyncMentionDispatch(convID, userID, sourceMessageID, content string, attachments []model.MessageAttachment) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), DefaultAgentTaskTimeout)
	defer cancel()
