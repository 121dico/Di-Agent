...
// OrchKBRAGRetriever is an optional capability implemented by KnowledgeService.
// Keeping it separate preserves compatibility with lightweight resolvers used in
// tests and lets KBBuilder fall back to the legacy whole-file context path.
type OrchKBRAGRetriever interface {
	SearchRAG(ctx context.Context, userID, kbID, query string, limit int) ([]KnowledgeRAGResult, error)
}
...
type OrchestratorService struct {
	...
	checkpointSessions AttachedCheckpointSessionSource
	checkpointStore    AttachedCheckpointSource
...
//     [Attachment, Blackboard, KB, AttachedCheckpoint, AgentConfig]
//     最终输出: agentConfig + attachedCheckpoint + kb + blackboard + attach
//   - WorkerChain（路径 C：dispatchSingleAgent）
//     [KB, Blackboard, AttachedCheckpoint, AgentConfig]
//     最终输出: agentConfig + attachedCheckpoint + blackboard + kbPreload
//   - OrchChain（路径 D：handleOrchestratedDispatch）
//     [KB, OrchestratorPrompt, AttachedCheckpoint, AgentConfig]
//     最终输出: agentConfig + attachedCheckpoint + orchPrompt + kbPreload
//
...
func NewOrchestratorServiceWithDeps(deps OrchestratorDeps) *OrchestratorService {
	svc := &OrchestratorService{
		...
		checkpointSessions: deps.CheckpointSessions,
...
func (s *OrchestratorService) buildDefaultChains() {
	...
	checkpoint := &AttachedCheckpointBuilder{Sessions: s.checkpointSessions, Checkpoints: s.checkpointStore}

	if s.directReplyChain == nil {
		// 路径 A：最终输出 agentConfig + attachedCheckpoint + kb + blackboard + attach
		s.directReplyChain = NewContextChain(attach, blackboard, kb, checkpoint, agentCfg)
	}
	if s.workerChain == nil {
		// 路径 C：最终输出 agentConfig + attachedCheckpoint + blackboard + kbPreload
		s.workerChain = NewContextChain(kb, blackboard, checkpoint, agentCfg)
	}
	if s.orchChain == nil {
		// 路径 D：最终输出 agentConfig + attachedCheckpoint + orchPrompt + kbPreload
		s.orchChain = NewContextChain(kb, orchPrompt, checkpoint, agentCfg)
	}
	if s.fanoutChain == nil {
		// 路径 C 异步 fanout 变体：[Frame, KB, AttachedCheckpoint, AgentConfig]
		// 最终输出 agentConfig + attachedCheckpoint + kbPreload + frame
		s.fanoutChain = NewContextChain(fanoutFrame, kb, checkpoint, agentCfg)
	}
