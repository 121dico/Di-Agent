...
type KnowledgeService struct {
	kbRepo            repository.KnowledgeStore
...
func NewKnowledgeService(kbRepo repository.KnowledgeStore, userRepo *repository.UserRepo, uploadDir, publicBaseURL string) *KnowledgeService {
	if uploadDir == "" {
		uploadDir = "./uploads"
	}
...
func (s *KnowledgeService) UploadFile(ctx context.Context, userID, kbID string, fileHeader *multipart.FileHeader) error {
	...
	created, err := s.kbRepo.AddFile(ctx, kbID, safeName, dbPath, fileHeader.Size, mimeType, hashHex, previewText, previewType)
	if err != nil {
		return err
	}
	// Indexing is deliberately best-effort: the original file and preview are
	// already durable, while any RAG failure is exposed through rag_status and
	// can be recovered through the reindex endpoint.
	s.indexFileBestEffort(ctx, created)
	return nil
}
