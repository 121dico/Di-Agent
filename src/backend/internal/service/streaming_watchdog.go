...
// 避免 Codex 等无增量输出的 CLI 在仍执行时被误判为失败。
const DefaultStreamingWatchdogMaxAge = DefaultAgentTaskTimeout + time.Minute
...
// 用法：
//
//	wd := NewStreamingWatchdog(msgRepo, logger, DefaultStreamingWatchdogMaxAge, 10*time.Second)
...
// NewStreamingWatchdog 构造 watchdog。interval 通常 10s。
func NewStreamingWatchdog(msgRepo MessageRepoStreaming, logger *slog.Logger, maxAge, interval time.Duration) *StreamingWatchdog {
	if maxAge <= 0 {
		maxAge = DefaultStreamingWatchdogMaxAge
	}
	if interval <= 0 {
