...
func TestExtractCardsFromContent(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expectedStrippedNotContain: []string{"```di_agent", "\"cards\""},
		...
		{
			name: "retired_fence_is_read_while_new_prompts_write_canonical_fence",
			input: "before\n```agenthub\n" + // [brand-compat]
				`{"cards":[{"type":"info","id":"legacy1"}]}` + "\n```\nafter",
			expectedCardsLen:           1,
			expectedStrippedContains:   []string{"[CARD:legacy1]", "before", "after"},
			expectedStrippedNotContain: []string{"```agenthub", `"cards"`}, // [brand-compat]
			expectedCardsJSONContains:  []string{`"id":"legacy1"`},
		},
		{
		...
		},
		{
			...
			input:                      "开头\n```di_agent\nthis is not json{]\n```\n结尾",
			expectedCardsLen:           0,
			expectedStrippedContains:   []string{"```di_agent", "this is not json{]", "结尾"},
...
// 与 agent 正文 ```di_agent{"cards":[...]}``` block 解析出的卡片被合并、剥离后存入 DB。
func TestCreateAgentReplyMergesDaemonAndTextCards(t *testing.T) {
	...
	if strings.Contains(agentMsg.Content, "```di_agent") {
		t.Errorf("content should not contain fenced di_agent block, got: %s", agentMsg.Content)
	}
