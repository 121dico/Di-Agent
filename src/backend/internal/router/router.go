...
type Deps struct {
	...
	CheckpointHandler          *handler.ConversationCheckpointHandler
	ContextHandler             *handler.ConversationContextHandler
...
func Setup(r *gin.Engine, deps Deps) {
	...
	apiGroup.Use(authMiddleware)
	{
		...
		kbRoutes := apiGroup.Group("/knowledge-bases")
		{
			...
			kbRoutes.GET("/:id/rag/search", deps.KnowledgeHandler.SearchRAG)
			kbRoutes.POST("/:id/reindex", deps.KnowledgeHandler.Reindex)
		...
		convRoutes.Use(
			middleware.ValidateUUIDParam("id"),
			middleware.ValidateUUIDParam("messageId"),
			middleware.ValidateUUIDParam("checkpointId"),
		)
		{
			...
			convRoutes.GET("/:id/checkpoints", deps.CheckpointHandler.List)
			convRoutes.POST("/:id/checkpoints", deps.CheckpointHandler.Create)
			convRoutes.GET("/:id/checkpoints/:checkpointId", deps.CheckpointHandler.Get)
			convRoutes.DELETE("/:id/checkpoints/:checkpointId", deps.CheckpointHandler.Delete)
			convRoutes.POST("/:id/checkpoints/:checkpointId/continue", deps.ContextHandler.Continue)
			convRoutes.POST("/:id/checkpoint-imports", deps.ContextHandler.Import)
			convRoutes.POST("/:id/forks", deps.ContextHandler.Fork)
			convRoutes.GET("/:id/fork", deps.ContextHandler.GetFork)
			convRoutes.GET("/:id/context-usage", deps.ContextHandler.ListUsage)
		}
		...
		if deps.ReportHandler != nil {
			...
			reportRoutes.Use(middleware.ValidateUUIDParam("id"), middleware.ValidateUUIDParam("runId"))
			{
				...
				reportRoutes.GET("/:id/order-cohort", deps.ReportHandler.QueryOrderCohort)
				...
				reportRoutes.GET("/:id/station-score-evidence", deps.ReportHandler.QueryStationScoreEvidence)
	...
	mcpGroup.Use(middleware.MCPAuth(deps.DaemonToken))
	{
		...
		mcpGroup.GET("/knowledge-bases/:id/rag/search", deps.KnowledgeHandler.SearchRAG)
