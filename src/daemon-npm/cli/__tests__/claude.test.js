...
test('claude buildCommand adds configured overload fallback model', () => {
  ...
  try {
    ...
    const spec = createClaudeCliSpec(ctx);
    const command = spec.buildCommand({ cli_tool: 'claude' }, {
      command: 'claude',
      systemPrompt: '',
      userPrompt: 'hello',
    ...
    const index = command.args.indexOf('--fallback-model');
...
test('claude.spawnPersistent.sendPrompt: clears timeout after a completed turn', async () => {
  ...
  ctx.spawn = () => ({
    ...
    stdout: {
      setEncoding() {},
      on(_event, listener) { stdoutListener = listener; },
    },
  ...
  try {
    ...
    const pendingPromise = result.sendPrompt('hello');
    await new Promise((resolve) => setImmediate(resolve));
    stdoutListener(JSON.stringify({ type: 'result', result: 'done' }) + '\n');
    await pendingPromise;

    assert.strictEqual(scheduledDelay, ctx.EXEC_TIMEOUT_MS);
    assert.strictEqual(clearedHandle, timerHandle);
  } finally {
...
test('claude.spawnPersistent adds configured overload fallback model', () => {
  ...
  try {
    ...
    const args = calls.spawn[0].args;
    const index = args.indexOf('--fallback-model');
    assert.notStrictEqual(index, -1);
