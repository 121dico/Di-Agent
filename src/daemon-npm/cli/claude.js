...
function createClaudeCliSpec(ctx) {
  ...
  return {
    ...
    buildCommand(task, deps) {
      ...
      const slotKey = typeof deps.runtimeAgentKey === 'function'
        ? deps.runtimeAgentKey(task.agent_id, task.conversation_id)
        : task.agent_id;
      const persistentEntry = task.agent_id
        ? (deps.runningAgents.get(slotKey) || deps.runningAgents.get(task.agent_id))
        : null;
      const persistent = Boolean(persistentEntry);
