...
function resolveAgentTimeoutMs(rawValue) {
  if (rawValue === undefined || rawValue === null || String(rawValue).trim() === '') {
    return DEFAULT_AGENT_TIMEOUT_MS;
  }
  const parsed = Number(rawValue);
  return Number.isSafeInteger(parsed) && parsed > 0 ? parsed : DEFAULT_AGENT_TIMEOUT_MS;
}
...
function runtimeAgentKey(agentId, conversationId) {
...
function stopRuntimeSlot(slotKey) {
  ...
  try {
    ...
    logFlow('info', 'agent.process_killed', {
      agent_id: entry.agentId || slotKey,
      session_id: entry.sessionId,
      pid: entry.process.pid,
      conversation_id: entry.currentConversationId,
    });
  } catch { /* already dead */ }
  runningAgents.delete(slotKey);
}

function stopAgentProcess(agent_id) {
  for (const [slotKey, entry] of runningAgents.entries()) {
    if (slotKey === agent_id || entry.agentId === agent_id) {
...
async function dispatchToPersistentSlot(ws, agentId, conversationId, userId, prompt, systemPrompt, taskCtx, cliTool = 'claude', onEvent = null, forceFresh = false) {
  ...
  if (forceFresh) {
    ...
    saveSessionMap();
    logFlow('info', 'agent.session_rollover', {
      agent_id: agentId,
...
async function handleTaskDispatch(ws, data) {
  ...
  if (task.force_fresh_session) {
    ...
    conversationSessions.delete(sessionKeyForTask(task));
    saveSessionMap();
  }
  if (!task.id) {
...
bus.on('task.cancel', ({ data } = {}) => {
  const { task_id, agent_id, conversation_id } = data || {};
  logFlow('info', 'task.cancel_received', { task_id, agent_id, conversation_id });
  if (!agent_id) return;
  const slot = runningAgents.get(runtimeAgentKey(agent_id, conversation_id)) || runningAgents.get(agent_id);
