  ...
  return <><section className={styles.card} aria-label="用户复购与人群重合">
    ...
    {!data ? <p className={styles.note} role="status">{loading ? '正在读取缓存或比对真实用户身份，页面不接收 DUID 明细…' : error ? '未能加载复购结果，请重试。' : valid ? '正在准备加载复购结果…' : '请选择有效消费区间。'}</p> : <>
      ...
      <div className={styles.metrics}>
        ...
        <article><span>至少 2 笔消费用户</span><strong>{stationNumber(data.repeat_users)}</strong><small>{stationPercent(stationRatio(data.repeat_users, data.users))} · 可含同日多单</small></article>
        <article><span>至少 2 天消费用户</span><strong>{stationNumber(data.multi_day_users)}</strong><small>{stationPercent(stationRatio(data.multi_day_users, data.users))} · 跨天回访</small></article>
      ...
      <p className={styles.note}>下图与下表使用消费当日等级。默认极高价敏：“过去7天同等级”要求同一 DUID 过去7天也以该等级消费，避免把标签迁移当作新人。</p>
