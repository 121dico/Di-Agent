...
export interface AgentChatConfig {
  agentName: string;
  workspace: 'report' | 'delivery';
  contextLabel: string;
  ensureSession: (userId: string, identity: string, isCurrent: () => boolean) => Promise<ReportAgentSession>;
  replaceContext: (manualContext: string, context: ReportAgentContext) => string;
}

export const reportAgentConfig: AgentChatConfig = {
  agentName: '报表agent', workspace: 'report', contextLabel: '当前报表', ensureSession: ensureReportAgentSession,
  replaceContext: replaceReportAgentContext,
};

interface Props { context: ReportAgentContext; onClose: () => void; active?: boolean; config?: AgentChatConfig }

export function ReportAgentChat({ context, onClose, active = true, config = reportAgentConfig }: Props) {
  ...
  useEffect(() => {
    ...
    if (!userId || !token) { setLoading(false); setError(`请先登录后使用${config.agentName}`); return; }
    void (async () => {
      try {
        const next = await config.ensureSession(userId, token, isCurrent);
        ...
        if (!disposed) setError(failure instanceof Error ? failure.message : `连接${config.agentName}失败`);
      } finally { if (!disposed) setLoading(false); }
    })();
    return () => { disposed = true; };
  }, [userId, token, attempt, config]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!isCurrent()) return;
      await updateConversationBlackboard(id, config.replaceContext(blackboard.manual_context ?? '', snapshot));
      if (!isCurrent()) return;
  ...
  const selectAgent = async (agentId: string) => {
    ...
    try {
      const conversation = await getOrCreateAgentChat(agentId, config.workspace, true);
  ...
  return <section className={styles.panel} aria-label={`${config.agentName}对话`}>
    <header className={styles.header}>
      <span className={styles.avatar}><Bot size={19} /></span>
      <div className={styles.heading}><strong>{config.agentName}</strong></div>
      <button className={styles.iconButton} onClick={onClose} aria-label={`收起${config.agentName}`} title="收起"><Minus size={18} /></button>
    </header>
    <label className={styles.agentPicker}>执行 Agent
      <select aria-label={config.workspace === 'report' ? '报表执行 Agent' : `${config.agentName}执行 Agent`} value={session?.agent.id ?? ''} disabled={loading || busy || !session} onChange={(event) => void selectAgent(event.target.value)}>
    ...
    <div className={styles.context} title={[context.reportName, context.dateRange, context.dataDate, context.cities.join('、') || '全部城市'].filter(Boolean).join(' · ')}>
      <span>{config.contextLabel}</span><strong>{context.reportName}</strong><small>{[context.dateRange, context.dataDate && `快照 ${context.dataDate}`, context.cities.join('、') || '全部城市'].filter(Boolean).join(' · ')}</small>
    </div>
    {loading ? <div className={styles.empty} role="status">正在连接{config.agentName}…</div> : session ? <>
      {messages.length === 0 ? <div className={styles.empty}><Bot size={30} /><strong>一起看看这份{config.contextLabel.replace('当前', '')}</strong><p>可以问我指标口径、数据变化，或当前筛选下的趋势。</p></div> : null}
    ...
    {session && (!online || !connected) && <div className={styles.notice} role="status"><span>{!connected ? '聊天连接已断开，正在重连…' : `${config.agentName}暂时离线，请恢复运行环境后重试。`}</span>{!online && <button onClick={() => setAttempt((value) => value + 1)}>重新连接</button>}</div>}
    {session && <div className={styles.composer}>
      <textarea ref={inputRef} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={`问问${config.agentName}…`} aria-label={`向${config.agentName}提问`} rows={2} maxLength={12000}
      ...
      <div className={styles.composerBottom}><small>{busy ? `${config.agentName}正在处理，请稍候` : 'Enter 发送 · Shift + Enter 换行'}</small>
        {streaming ? <button className={styles.sendButton} onClick={() => void stop()} aria-label={`停止${config.agentName}回复`}><Square size={14} /></button>
          : <button className={styles.sendButton} onClick={() => void handleSend()} disabled={!draft.trim() || busy || !online || !connected} aria-label={`发送给${config.agentName}`}><ArrowUp size={18} /></button>}
      </div>
    </div>}
  </section>;
}
