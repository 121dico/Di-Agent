...
export function stripKnowledgeRefs(text: string): string {
  knowledgeRefRe.lastIndex = 0;
  return text
    .replace(knowledgeRefRe, ' ')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n[ \t]+/g, '\n')
    .replace(/[ \t]{2,}/g, ' ')
    .replace(/\n{3,}/g, '\n\n')
...
export function appendKnowledgeRefs(
  text: string,
  refs: ReadonlyArray<Pick<KnowledgeReference, 'username' | 'kbName'>>,
): string {
  const existingKeys = new Set(extractKnowledgeRefs(text).map((ref) => ref.key));
  const suffix: string[] = [];
  for (const ref of refs) {
    const username = ref.username.trim();
    const kbName = ref.kbName.trim();
    const key = `${username}/${kbName}`;
    if (!username || !kbName || existingKeys.has(key)) continue;
    existingKeys.add(key);
    suffix.push(`{{${key}}}`);
  }
  const trimmed = text.trim();
  if (suffix.length === 0) return trimmed;
