...
describe('knowledgeReferenceState', () => {
  ...
  it('persists protocol references while exposing a clean visible body', () => {
    const content = appendKnowledgeRefs('请比较发布流程', [
      { username: 'alice', kbName: 'API文档' },
      { username: 'alice', kbName: 'API文档' },
      { username: 'bob', kbName: '运行手册' },
    ]);

    expect(content).toBe('请比较发布流程\n{{alice/API文档}} {{bob/运行手册}}');
    expect(stripKnowledgeRefs(content)).toBe('请比较发布流程');
  });

  it('supports a knowledge-only message without duplicating existing references', () => {
    expect(appendKnowledgeRefs('', [{ username: 'alice', kbName: 'API文档' }]))
      .toBe('{{alice/API文档}}');
    expect(appendKnowledgeRefs('正文 {{alice/API文档}}', [{ username: 'alice', kbName: 'API文档' }]))
      .toBe('正文 {{alice/API文档}}');
  });
