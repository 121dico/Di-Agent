...
function KnowledgeAttachmentList({ refs }: { refs: KnowledgeReference[] }): ReactNode {
  if (refs.length === 0) return null;
  return (
    <div className={styles.knowledgeAttachments} aria-label="知识库附件">
      {refs.map((ref) => (
        <div key={ref.key} className={styles.knowledgeAttachmentCard} title={`知识库：${ref.key}`}>
          <DatabaseOutlined className={styles.knowledgeAttachmentIcon} />
          <span className={styles.knowledgeAttachmentBody}>
            <span className={styles.knowledgeAttachmentName}>{ref.kbName}</span>
            <span className={styles.knowledgeAttachmentMeta}>{ref.username} · RAG 知识库</span>
          </span>
        </div>
      ))}
    </div>
...
/** Process message text leaves so @mentions render as first-class inline tokens. */
...
const MessageBubbleInner: React.FC<MessageBubbleProps> = ({
  ...
  const knowledgeAttachments = useMemo(
    () => extractKnowledgeRefs(message.content ?? ''),
    [message.content],
  );
  const displayContent = useMemo(
    () => stripKnowledgeRefs(message.content ?? ''),
    [message.content],
  );
      ...
      >
        ...
        <div className={styles.content}>
          ...
          >
            ...
            <KnowledgeAttachmentList refs={knowledgeAttachments} />
            {hasBlocks ? (
