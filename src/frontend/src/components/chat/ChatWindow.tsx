...
export const ChatWindow: React.FC<ChatWindowProps> = ({ onOpenPersonalReport, personalReportOpen = false }) => {
  ...
  useEffect(() => {
    ...
    getConversationFork(conversationId)
      .then((lineage) => {
        if (activeIdRef.current === conversationId) setForkLineage(lineage);
      })
      .catch(() => {
        if (activeIdRef.current === conversationId) setForkLineage(null);
      });
  }, [activeId]);
  ...
  const contextAgents = useMemo(() => {
    if (activeConv?.type === 'group') {
      return conversationAgents.map((agent) => ({ id: agent.agent_id, name: agent.name }));
    }
    if (activeConv?.type === 'agent' && activeConv.peer_id) {
      const agent = agents.find((item) => item.id === activeConv.peer_id);
      return [{ id: activeConv.peer_id, name: agent?.name ?? activeConv.peer_name ?? activeConv.title }];
    }
    return [];
  }, [activeConv, agents, conversationAgents]);
              ...
              <Tooltip title={`来源：${forkLineage.parent_title || '父对话'}`}>
                <Button
                  ...
                  onClick={(event) => {
                    ...
                    setActiveConversation(forkLineage.parent_conversation_id);
                  }}
                >
                  Fork
                </Button>
