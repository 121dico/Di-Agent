...
/** 在知识库中检索语义相关片段；向量服务不可用时后端会降级为关键词检索。 */
export async function searchKnowledgeBaseRAG(
  kbId: string,
  query: string,
  limit = 6,
): Promise<KnowledgeRAGResult[]> {
  const params = new URLSearchParams({ q: query, limit: String(limit) });
  return await get<KnowledgeRAGResult[]>(`${BASE}/${kbId}/rag/search?${params.toString()}`);
}

/** 使用当前 RAG 配置重建知识库中全部文件的索引。 */
export async function reindexKnowledgeBase(kbId: string): Promise<KnowledgeReindexResult> {
  return await post<KnowledgeReindexResult>(`${BASE}/${kbId}/reindex`);
}
