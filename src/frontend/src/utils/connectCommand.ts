...
export function buildCommands(
  ...
  const npx = `npx ${quoteArg(DAEMON_PACKAGE_SPEC)} --server-url ${quoteArg(serverURL)} --api-key ${quoteArg(apiKey)}`;
  const trimmedPath = (daemonNPMPath ?? '').replace(/\/$/, '');
  const entry = `${trimmedPath}/bin/agenthub-daemon.js`;
  const node = `node ${quoteArg(entry)} --server-url ${quoteArg(serverURL)} --api-key ${quoteArg(apiKey)}`;
  return { npx, node };
}
